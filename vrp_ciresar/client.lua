vRPciresarC = {}
Tunnel.bindInterface("vRP_ciresar",vRPciresarC)
Proxy.addInterface("vRP_ciresar",vRPciresarC)
vRP = Proxy.getInterface("vRP")
vRPSciresar = Tunnel.getInterface("vRP_ciresar","vRP_ciresar")
...
function CreateCar(x,y,z,heading) -- van
    ...
    if HasModelLoaded(hash) then
		...
		SetVehicleNumberPlateText(veh,"Millzop")
                        ...
                        if IsControlJustPressed(1,51) then
                            if (Vdist(pos.x, pos.y, pos.z, carPos.x , carPos.y, carPos.z) <= 50.0) and hasJob and aajuns and DoesEntityExist(veh) then
                                if culege == false then
                                    table.remove(coordonate,i)
                                    vRP.playAnim({false, {task="PROP_HUMAN_PARKING_METER"}, false})
                                    ...
                                    vRP.stopAnim( {false})
                                        ...
                                        culege = true
                                        vRP.notify({"~o~Go to put the box with cherries in car"})
                                    end)
                                else
                                    vRP.notify({"~r~You are picking allready"})
                                end
                            else
                                vRP.notify({"~r~The car isn't near"})
                            end
                        end
                    end
                end
            ...
            elseif IsControlJustPressed(1,246) then
                if cursaf then
                    if DoesEntityExist(veh) then
                        ...
                        vRP.notify({"~r~You don't have a car"})
                    end
                else
                    vRP.notify({"~r~You haven't finished"})
                end
            end
        end      
    end
